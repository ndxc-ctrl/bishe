    ...
    def _setDrone(self):
        for i, item in enumerate(self.batch):
            ...
            try:
                ...
                # 确保无人机姿态是完全水平的，这样俯视相机就能完美正对下方
                isaac_debug_quat = np.array([1.0, 0.0, 0.0, 0.0])

                self.drone_prims[i].set_world_pose(position=debug_pos, orientation=isaac_debug_quat)

                print(f"⏳ [火焰调试拍照]: 正在目标上方悬停，预热 300 帧等待 RTX Flow 体积粒子爆发...")
                ...
                # 🟢 核心修复：狂转 300 帧，强制刷新物理与系统 UI，让Flow有足够的时间渲染计算
                for _ in range(300):
                    self.world.step(render=True)
                    app.update()

                self.world.render()  # 确保渲染管线冲洗完毕
                ...
                if rgba is not None:
                    ...
                    if img.ndim == 3 and img.shape[2] >= 3:
                        ...
                        print(f"🔥 [火焰调试拍照成功]: 目标上方俯视图已保存为 '{filename}'，快去根目录检查火烧起来没有！")
            except Exception as e:
