...
# =====================================================================
# 🚀 启动引擎前：直接将 Flow 和粒子插件注入到基础配置中
# =====================================================================
print("🔥 [引擎底层]: 正在初始化并挂载流体(Flow)、体积(Volume)与粒子扩展...")
config = {
    "headless": True,
    "active_gpu": 0,
    "physics_gpu": 0,
    ...
    "exts": [
        ...
        "omni.flowusd",  # 核心：火焰特效的依赖
        "omni.volume"    # 核心：体积渲染的依赖
    ]
}
simulation_app = SimulationApp(config)

# =====================================================================
# 🚀 终极黑客修复：强行打通无头模式下相机的 RTX 流体与后期特效捕获权限！
# =====================================================================
import carb
print("🔥 [引擎底层]: 正在暴力重写 RTX 后台渲染器配置...")
settings = carb.settings.get_settings()
settings.set_int("/rtx/rendermode", 0)  # 必须开启实时光追(0)，路径追踪下不支持部分Flow特性
settings.set_bool("/rtx/flow/enabled", True)  # 强制开启流体
settings.set_bool("/rtx/volume/enabled", True)  # 强制开启体积渲染
settings.set_bool("/rtx/particles/enabled", True)  # 强制开启粒子
settings.set_bool("/rtx/translucency/enabled", True)  # 强制开启透明通道（火光）
settings.set_bool("/rtx/post/enabled", True)  # 强制开启后期处理（发光辉光）

# 🟢 核心修复：允许 SyntheticData（相机传感器）捕获流体、粒子及后处理！
settings.set_bool("/omni/syntheticdata/renderer/enableLdr", True)
settings.set_bool("/omni/syntheticdata/renderer/enableFlow", True)
settings.set_bool("/omni/syntheticdata/renderer/enableParticles", True)
settings.set_bool("/omni/syntheticdata/renderer/enableVolume", True)
settings.set_bool("/omni/syntheticdata/renderer/enablePostProcess", True) # <- 这里是让相机能拍到Flow特效的最核心参数！
print("🔥 [引擎底层]: 所有特效渲染管线及相机捕获权限已强制上线！")
# =====================================================================
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            for t in range(2000):
                ...
                is_no_anomaly = "无异物" in qwen_down_caption or "无异常" in qwen_down_caption
                is_anomaly = ("有异物" in qwen_down_caption or "有异常" in qwen_down_caption or "火焰" in qwen_down_caption) and not is_no_anomaly

                if not is_anomaly and actions[0] == 'stop':
                    print("⚠️ [系统拦截]: 大模型在未发现异常时试图停止任务，已强制拦截并更改为向前探索！")
                    actions[0] = 'forward'
                    if steps_size[0] == 0.0:
                        steps_size[0] = 500.0
                ...
                if is_anomaly:
                    anomaly_target = qwen_down_caption.split("是")[-1].strip().strip(
                        '。.,，') if "是" in qwen_down_caption else "异常/火焰目标"
                    ...
                    try:
                        ...
                        while curr_z > target_z_40m and track_step < track_max_steps:
                            ...
                            🚨【锁定目标】：你要追踪的是【{anomaly_target}】！如果画面中有其他正常物体，请完全无视！

                            请严格判断该目标相对于画面绝对中心点的位置：
