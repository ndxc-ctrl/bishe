...
# =====================================================================
# 🚀 终极黑客修复：强行打通无头模式下被阉割的 RTX 流体与粒子特效管线！
# =====================================================================
import carb
from omni.isaac.core.utils.extensions import enable_extension

print("🔥 [引擎底层]: 正在强制挂载流体(Flow)、体积(Volume)与粒子扩展...")
enable_extension("omni.particle.system.core")
enable_extension("omni.particle.system.bundle")
enable_extension("omni.flowusd")  # 核心：火焰特效的依赖
enable_extension("omni.volume")  # 核心：体积渲染的依赖

print("🔥 [引擎底层]: 正在暴力重写 RTX 后台渲染器配置...")
settings = carb.settings.get_settings()
settings.set_int("/rtx/rendermode", 0)  # 必须开启实时光追(0)，Flow特效在路径追踪下可能会失效
...
settings.set_bool("/rtx/translucency/enabled", True)  # 强制开启透明通道（火心）
settings.set_bool("/rtx/post/enabled", True)  # 强制开启后期处理（发光辉光）
settings.set_bool("/omni/syntheticdata/renderer/enableLdr", True)  # 确保合成数据相机能抓取特效
print("🔥 [引擎底层]: 所有特效渲染管线已强制上线！")
# =====================================================================
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            for t in range(999999):
                ...
                is_anomaly = (
                                         "有异物" in qwen_down_caption or "有异常" in qwen_down_caption or "火焰" in qwen_down_caption) and not is_no_anomaly
